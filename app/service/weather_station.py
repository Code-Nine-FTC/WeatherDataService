...
class WeatherStationService:
    ...
    async def update_station(self, station_id: int, data: WeatherStationCreate, user_id: int) -> None:
        result = await self._session.execute(
        ...
        if user_id != self.user:
            raise HTTPException(status_code=403, detail="Usuário não tem permissão para editar esta estação")
