...
def test_create_station(authenticated_client: TestClient, parameter_types_fixture):
    data = {
        "name": "Estação Nova",
        "uid": "nova-uid-123",
        "latitude": -10.0,
...
def test_create_station_with_existing_uid(
    authenticated_client: TestClient, station_with_existing_uid_fixture
):
...
def test_create_station_with_missing_fields(authenticated_client: TestClient):
...
def test_create_station_with_parameters_and_verify_link(
    authenticated_client: TestClient, parameter_types_fixture
):
    data = {
        "name": "Vinculada",
        "uid": "com-parametro",
...
def test_list_stations(authenticated_client: TestClient, full_station_fixture):
...
def test_list_stations_with_filters(authenticated_client: TestClient, full_station_fixture):
...
def test_get_parameter_by_invalid_type_id(authenticated_client: TestClient):
    response = authenticated_client.get("/stations/parameters/99999")
    assert response.status_code == status.HTTP_404_NOT_FOUND


# 9. GET - Estação por ID precisa de fixture
def test_get_station_by_id(authenticated_client: TestClient, full_station_fixture):
    station = full_station_fixture["station"]
...
def test_get_station_by_invalid_id(authenticated_client: TestClient):
...
def test_update_station_with_new_parameters(
    authenticated_client: TestClient, full_station_fixture
):
    station = full_station_fixture["station"]
...
def test_update_station_invalid_id(authenticated_client: TestClient):
    data = {"name": "Falha"}
